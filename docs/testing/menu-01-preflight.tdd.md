...
| 锁定实例与档案配对：有 `DistroName` 匹配的档案时自动切换到该档案并以其身份预检；无匹配档案时 Enter/CursorRight/执行动作全部被阻止且锁保持，不 arm 压缩请求 | `CompactionTargetProfileFactoryTests.FindProfileForTarget_returns_the_profile_whose_distro_matches_case_insensitively` 等 5 个 fact、`VelaTerminalShellTests.Missing_profile_blocks_a_locked_installed_target_from_execution` | 单元 / 交互 |
