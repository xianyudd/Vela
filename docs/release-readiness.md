...
- [x] 真实截图序列 Demo GIF。
- [x] 目标选择、目标锁定、预检、日志归档和预计可回收空间的文档说明。

## 首个 Release 前必须完成

- [ ] 录制一次真实的只读交互 GIF：实例列表 → Enter → 预检详情 → Esc → 日志归档。
- [ ] 捕获真实影响评估页，展示目标发行版、当前体积、访客已用空间和“预计可回收空间”数值。
- [ ] 捕获两次 `Y` 确认页，确认页面只展示当前锁定目标。
...
- [x] 添加 CI 状态徽章，并在 README 标明当前 private preview 与 Release 状态。
